...
public class AuraApplication {
    ...
    // The shared Claude client, exposed as a singleton bean so ConversationService (and any future
    // service) can constructor-inject it instead of constructing its own. fromEnv() reads
    // ANTHROPIC_API_KEY from the environment, keeping the key out of source. Spring manages the
    ...
    // round-trip to the Claude API was wired up before any service layer existed. Day 2 supersedes
    // it with ConversationRunner, which exercises real multi-turn state via ConversationService.
    //
