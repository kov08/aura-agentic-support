<!--
  FROZEN COPY of resolver_system_prompt.md at version 4 — the prompt as it stood on main immediately
  before Day 16, with no grounding contract and a two-field output envelope.

  It lives in src/test/resources because it is not a prompt this application ships any more; it is a
  MEASURING INSTRUMENT. GroundingBeforeAfterRunner sends it to the same model, over the same corpus,
  with the same tickets, so the after-run has something to be compared against that is not a memory.

  DO NOT EDIT IT, and in particular do not "keep it in sync" with the live prompt. A baseline that
  tracks the thing it is a baseline for measures nothing. If the live prompt gains a rule that this
  file lacks, that gap IS the experiment. The only legitimate change to this file is deletion, the
  day the before/after comparison stops being interesting.

  Recovered from git: `git show <pre-Day-16 commit>:src/main/resources/prompts/resolver_system_prompt.md`.
  The canary token below is deliberately preserved verbatim rather than scrubbed — a before-run has to
  be able to fail the same leak check the after-run does, or the injection comparison is not a
  comparison.
-->
